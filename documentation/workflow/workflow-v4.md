...
2. Graph Construction Service (builds ownership/control graph)  
   - Uses AI: No  
   - Input: Structured ownership relationships derived from Phase 0 packet and registry filings  
   - Output: Ownership/control graph representation  

3. **Artifact Generator: Ownership + UBO** (stores ART-6 and generates ART-7, ART-8, updates ART-5)  
   - Uses AI: Yes  
   - Input: Phase 0 packet + Phase 0 questionnaire bundle + registry filings + graph output  
   - Output:  
     - **ART-6-OwnershipGraphRecord** (from Graph Construction Service output)  
     - **ART-7-BeneficialOwnerSummary**  
     - **ART-8-OwnershipGapReport**  
